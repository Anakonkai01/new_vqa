...
Usage in notebook:
    ...
    ivqa = InteractiveVQA(loaded_models, q_vocab, a_vocab, VAL_IMAGE_DIR, device=DEVICE)
...
class InteractiveVQA:
    ...
    def __init__(self, models, q_vocab, a_vocab, image_dir,
        ...
        Args:
            ...
            a_vocab: Vocabulary for answers (decoder output)
    ...
    def ask(self, question, show_plot=True):
        ...
        q_tensor = torch.tensor(
            self.q_vocab.numericalize(question), dtype=torch.long
        )

        print(f"Image   : {os.path.basename(self.current_img_path)}")
        print(f"Question: {question}")
        print(f"Tokens  : {self.q_vocab.tokenize(question)}")
        print()
            ...
            for m in sorted(self.models.keys()):
                ...
                if use_attn:
                    g = greedy_decode_with_attention(
                        model, img_tensor, q_tensor, self.a_vocab, device=self.device)
                else:
                    g = greedy_decode(
                        model, img_tensor, q_tensor, self.a_vocab, device=self.device)
                ...
                if use_attn:
                    b = beam_search_decode_with_attention(
                        model, img_tensor, q_tensor, self.a_vocab,
